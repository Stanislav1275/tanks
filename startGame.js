class Game {
    ...
    rebound(box1, box2) {//cтолкнулся ли танк с конкретным объектом
        ...
        let by1 = ((Bx1 - Bp1) * Math.cos(bTheta) - (By1 - Bq1) * Math.sin(bTheta)) + Bq1;
        //
        let bx2 = ((Bx2 - Bp1) * Math.cos(bTheta) - (By2 - Bq1) * Math.sin(bTheta)) + Bp1;
        let by2 = ((Bx2 - Bp1) * Math.cos(bTheta) - (By2 - Bq1) * Math.sin(bTheta)) + Bq1;
        //
        let bx3 = ((Bx3 - Bp1) * Math.cos(bTheta) - (By3 - Bq1) * Math.sin(bTheta)) + Bp1;
        let by3 = ((Bx3 - Bp1) * Math.cos(bTheta) - (By3 - Bq1) * Math.sin(bTheta)) + Bq1;
        //
        let bx4 = ((Bx4 - Bp1) * Math.cos(bTheta) - (By4 - Bq1) * Math.sin(bTheta)) + Bp1;
        let by4 = ((Bx4 - Bp1) * Math.cos(bTheta) - (By4 - Bq1) * Math.sin(bTheta)) + Bq1;
        ///
        let theta = box1.deg * oneRad
        let p1 = box1.x + box1.w / 2;
        let q1 = box1.y + box1.w / 2;

        let x1 = box1.x;
        let y1 = box1.y;
        //
        let x3 = box1.x + box1.w;
        let y3 = box1.y;
        //
        let x2 = box1.x;
        let y2 = box1.y + box1.h;
        //
        ...
        //
        let p1x3 = ((x2 - p1) * Math.cos(theta) - (y2 - q1) * Math.sin(theta)) + p1;
        let p1y3 = ((x2 - p1) * Math.cos(theta) + (y2 - q1) * Math.sin(theta)) + q1;
        //
        let p1x2 = ((x3 - p1) * Math.cos(theta) - (y3 - q1) * Math.sin(theta)) + p1;
        let p1y2 = ((x3 - p1) * Math.cos(theta) + (y3 - q1) * Math.sin(theta)) + q1;
        //
