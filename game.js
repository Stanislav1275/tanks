...
walls = [
    ...
    new Wall(10, 0, canvas.width, 40),
    new Wall(window.innerWidth - 40, 0, 40, canvas.height),
    new Wall(10, canvas.clientHeight - 40, canvas.width, 40),
    new Wall(10, canvas.clientHeight / 2 - 40, canvas.width / 3, 40),
    new Wall(canvas.clientWidth / 2, canvas.clientHeight / 3, 40, canvas.height / 2 - 40),
]
        ...
        for (let wall of walls) {
            for (let bullet of bullets) {
                if ((bullet.x >= wall.x) && (bullet.x + bullet.w <= wall.x + wall.w) && (bullet.y >= wall.y) && (bullet.y + bullet.h <= wall.y + wall.h)) {
                    ...
                    let wallHalfW = bullet.w / 2;
                    let wallHalfH = bullet.h / 2;
                    let bulletCenterX = bullet.x + bulletHalfW;
                    let bulletCenterY = bullet.y + bulletHalfH;
                    let wallCenterX = wall.x + wallHalfW;
                    let wallCenterY = wall.y + wallHalfH;
